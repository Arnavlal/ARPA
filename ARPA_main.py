...
if ARGS.dont_split_paralogs==True:
    ...
    for i in np.arange(len(most_represented+1)):
        if i>0:
            #TIMING
            a  = seq_w_genome[np.where(seq_w_genome[:,1]==i)[0],:]
            Homolog_Split[a[:,3].astype(int),i-1] = a[:,2]
            #TIMING
    ...
    print("Finished Addition of alle scores to File: --- %s seconds ---" % (time.time() - start_time)) #TIME
