...
PARSER = argparse.ArgumentParser(
    prog="ARPA_MCL_Clust.py",
    description="Alignment-Free Residue Pangenomic Analysis: MCL_clustering")
PARSER.add_argument(
    "-v",
    "--version",
...
if len(sys.argv) == 1:
    ...
    print("")
    print("ARPA_MCL_Clust.py -ct 0.95 /user/path/to/faa/files/")
    print("")
    print("")
    sys.exit(0)
...
if ARGS.output_file:
    if ARGS.output_file.endswith("/"):
        os.mkdir(str(ARGS.output_file) + "ARPA_results")
        RESULTS_FOLDER = str(ARGS.output_file) + "ARPA_results{}".format(OS_SEPARATOR)
    else:
        os.mkdir(str(ARGS.output_file) + str(OS_SEPARATOR) + "ARPA_results")
        RESULTS_FOLDER = str(ARGS.output_file) + str(OS_SEPARATOR) + "ARPA_results{}".format(OS_SEPARATOR)
else:
    
    os.mkdir("ARPA_results") #make a new directory in the working directory to store analysis files
    RESULTS_FOLDER = "ARPA_results{}".format(OS_SEPARATOR) #results folder uses he current timestring to create a new file that is time-stamped
...
print("Creating Indexing File and Performing MCL Pairwise comparisons")
print(" ")
print(" ")
...
id_e = 0
cluster = collections.defaultdict(list)
for i in itv_clust.keys():
    group = itv_clust[i][0]
    for j in np.arange(len(group)):
        cluster[id_e].append(identity[group[j]])
    reps.append(vrcs[group[0]])
    id_e +=1

import pickle
with open(os.path.join(RESULTS_FOLDER,"Rep_link.pkl"),"wb") as output_file:
    pickle.dump(cluster,output_file,  pickle.HIGHEST_PROTOCOL)

comparison_group = np.array(reps)[:,0,:]
comparisons = np.zeros((id_e*id_e, 3))
index = 0
for i in tqdm(np.arange(id_e)):
    comparison_pairwise = 1-np.sum(abs(comparison_group - comparison_group[i,:]), axis = 1)/np.sum(comparison_group[i,:])
    comparison_pairwise[comparison_pairwise<0.95] = 0
    comparisons[index:index+id_e,0] = i+1
    comparisons[index:index+id_e,1] = np.arange(1,id_e+1)
    comparisons[index:index+id_e,2] = comparison_pairwise
    index += id_e

np.savetxt(os.path.join(RESULTS_FOLDER,"MCL_input.txt"), comparisons[comparisons[:,2] !=0])


print("Created Output for clustering:--- %s seconds ---" % "{:.2f}".format(time.time() - start_time))
print(" ")
print(" ")
